...
class ElkapodControllerGui(Node):
    def __init__(self):
        ...
        self.map_info_subscriber = self.create_subscription(
            Info, map_info_topic, self._map_info_callback, 10)
        ...
        self.set_mode_localization_client = self.create_client(
            Empty, "/rtabmap/set_mode_localization")
        self.set_mode_mapping_client = self.create_client(
            Empty, "/rtabmap/set_mode_mapping")

        self.publish_map_client = self.create_client(
            PublishMap, "/rtabmap/publish_map")
        self.create_new_map_client = self.create_client(
            Empty, "/rtabmap/trigger_new_map")
...
#                self._service_done_callback)
    ...
    def send_slam_mapping_mode_cmd(self):
        ...
        if not result:
            ...
            goal = Empty.Request()
            self._send_goal_future = self.set_mode_mapping_client.call_async(
                goal)
#            self._send_goal_future.add_done_callback(
#                self._service_done_callback)

    def send_slam_localization_mode_cmd(self):
        result = self.set_mode_localization_client.wait_for_service(
            timeout_sec=3.0)
        if not result:
            # self.ros2_qt_bridge.send_async_cmd_signal.emit(False)
            print("Failed accessing for slam mode service")
            ...
            goal = Empty.Request()
            self._send_goal_future = self.set_mode_localization_client.call_async(
                goal)
#            self._send_goal_future.add_done_callback(
#                self._service_done_callback)
...
#                self._service_done_callback)
    ...
    def send_new_map_cmd(self):
        ...
        if not result:
            ...
            goal = Empty.Request()
            self._send_goal_future = self.create_new_map_client.call_async(
                goal)
#            self._send_goal_future.add_done_callback(
#                self._service_done_callback)
        ...
        try:
            ...
            if len(response.goals_canceling) > 0:
                self.get_logger().info(
                    f"Success! Canceling {len(response.goals_canceling)} goals.")
            else:
