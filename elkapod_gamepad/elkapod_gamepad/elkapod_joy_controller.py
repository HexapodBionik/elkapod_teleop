...
class ElkapodJoyNode(Node):
    ...
    def __init__(self):
        ...
        self.declare_parameter(
            ...
            None,
            ParameterDescriptor(
                description="Max roll value in rad", dynamic_typing=True
            ),
        )
        self.declare_parameter(
            "pitch.max_rad",
            None,
            ParameterDescriptor(
                description="Max pitch value in rad", dynamic_typing=True
            ),
        )
        self.declare_parameter(
            "max_vel.tripod",
            None,
            ParameterDescriptor(
                description="Max linear velocity for tripod gait", dynamic_typing=True
            ),
        )
        self.declare_parameter(
            "max_vel.wave",
            None,
            ParameterDescriptor(
                description="Max linear velocity for wave gait", dynamic_typing=True
            ),
        )
        self.declare_parameter(
            "max_vel.ripple",
            None,
            ParameterDescriptor(
                description="Max linear velocity for ripple gait", dynamic_typing=True
            ),
        )
        self.declare_parameter(
            ...
            None,
            ParameterDescriptor(
                description="Max angular velocity for tripod gait", dynamic_typing=True
            ),
        )
        self.declare_parameter(
            ...
            None,
            ParameterDescriptor(
                description="Max angular velocity for wave gait", dynamic_typing=True
            ),
        )
        self.declare_parameter(
            ...
            None,
            ParameterDescriptor(
                description="Max angular velocity for ripple gait", dynamic_typing=True
            ),
        )
        self.declare_parameter(
            "gamepad_model",
            "xbox-series-x",
            ParameterDescriptor(
                description="Model of gamepad use for control", dynamic_typing=True
            ),
        )
        ...
        self._base_height_max = (
            self.get_parameter("base_height.max").get_parameter_value().double_value
        )
        self._base_height_default = (
            self.get_parameter("base_height.default").get_parameter_value().double_value
        )
        self._roll_max_rad = (
            self.get_parameter("roll.max_rad").get_parameter_value().double_value
        )
        self._pitch_max_rad = (
            self.get_parameter("pitch.max_rad").get_parameter_value().double_value
        )
        self._max_vel_tripod = (
            self.get_parameter("max_vel.tripod").get_parameter_value().double_value
        )
        self._max_vel_wave = (
            self.get_parameter("max_vel.wave").get_parameter_value().double_value
        )
        self._max_vel_ripple = (
            self.get_parameter("max_vel.ripple").get_parameter_value().double_value
        )
        self._max_angular_vel_tripod = (
            self.get_parameter("max_angular_vel.tripod")
            .get_parameter_value()
            .double_value
        )
        self._max_angular_vel_wave = (
            self.get_parameter("max_angular_vel.wave")
            .get_parameter_value()
            .double_value
        )
        self._max_angular_vel_ripple = (
            self.get_parameter("max_angular_vel.ripple")
            .get_parameter_value()
            .double_value
        )
        self._gamepad_model = (
    ...
    def _joystick_callback(self, msg: Joy):
        ...
        if angular_clockwise_trigger > 0.0:
            self._speed.omega = (
                0.5 * pow(angular_clockwise_trigger, 3) * self._max_angular_speed
            )
            self._speed.vx = 0.0
            self._speed.vy = 0.0
        elif angular_anticlockwise_trigger > 0.0:
            self._speed.omega = (
                -0.5 * pow(angular_anticlockwise_trigger, 3) * self._max_angular_speed
            )
            self._speed.vx = 0.0
            self._speed.vy = 0.0
        ...
        if pitch_roll_reset_btn:
            ...
            self._pitch = 0.0
            self.get_logger().info(
                "Roll and pitch of the main body moved to zero positions"
            )

        new_pitch = self._pitch + self._ROLL_PITCH_STEP * pitch_axis
        new_roll = self._roll - self._ROLL_PITCH_STEP * roll_axis

        x = pow(new_roll, 2) / pow(self._roll_max_rad, 2) + pow(new_pitch, 2) / pow(
            self._pitch_max_rad, 2
        )
        ...
        if (
            change_walk_mode_btn
            and self._speed.norm() == 0.0
            and not self._change_walk_pressed
        ):
